package com.doodt.m3u8.download;

import com.doodt.m3u8.Exception.M3u8Exception;
import com.doodt.m3u8.utils.Constant;
import com.doodt.m3u8.utils.EchoUtil;
import com.doodt.m3u8.utils.MediaFormat;
...
import static com.doodt.m3u8.utils.Constant.FILESEPARATOR;
...
public class M3u8DownloadFactory {
    private static Logger logger = LoggerFactory.getLogger(M3u8DownloadFactory.class);
    ...
    public static class M3u8Download {
        ...
        private boolean startDownload() {
            ...
            if (!file1.exists())
                file1.mkdirs();
            logger.info("开始下载:{},url:{}", fileName, DOWNLOADURL);
            ...
            try {
                boolean flag = fixedThreadPool.awaitTermination(5, TimeUnit.MINUTES);
                if (flag && downloadComp) {
                    logger.info("下载完成:{},大小:{},耗时:{}秒", fileName, EchoUtil.convertToDownloadSpeed(downloadBytes, 3), (System.currentTimeMillis() - starttime) / 1000);
                    //开始合并视频
                    mergeTs();
                    //删除多余的ts片段
                    deleteFiles();
        ...
        private Thread getThread(String urls, int i) {
            return new Thread(() -> {
                ...
                while (count <= retryCount) {
                    try {
                        //中文转码
                        String encodeUrl = HttpUtils.encordHttpUrl(urls);
                        //模拟http请求获取ts片段文件
                        ...
                        logger.info("第" + count + "获取链接重试！\t" + urls);
                        count++;
                        e.printStackTrace();
                    } finally {
                        try {
                ...
                finishedCount++;
//                logger.info(urls + "下载完毕！\t已完成" + finishedCount + "个，还剩" + (tsSet.size() - finishedCount) + "个！");
            });
        }
        ...
        private String getKey(String url, StringBuilder content) {
            ...
            for (int i = 0; i < split.length; i++) {
                ...
                if (s.contains("#EXTINF")) {
                    String s1 = split[++i].trim();
                    tsSet.add(EchoUtil.isUrl(s1) ? s1 : mergeUrl(relativeUrl, s1));
                }
            }
            if (!StringUtils.isEmpty(key)) {
                key = key.replace("\"", "");
                return getUrlContent(EchoUtil.isUrl(key) ? key : mergeUrl(relativeUrl, key), true).toString().replaceAll("\\s+", "");
            }
            return null;
        }
        ...
        private StringBuilder getUrlContent(String urls, boolean isKey) {
            ...
            while (count <= retryCount) {
                try {
                    ...
                    count++;
                    lastError = "获取" + DOWNLOADURL + "内容异常:" + e.getMessage();
                } finally {
                    if (httpURLConnection != null) {
                        httpURLConnection.disconnect();
                    }
            ...
            if (count > retryCount)
                throw new M3u8Exception(lastError);
            return content;
        }
        ...
        private byte[] decrypt(byte[] sSrc, int length, String sKey, String iv, String method) throws Exception {
            ...
            byte[] ivByte;
            if (iv.startsWith("0x"))
                ivByte = EchoUtil.hexStringToByteArray(iv.substring(2));
    ...
    public static void destroied() {
        m3u8Download.BLOCKING_QUEUE.clear();
        m3u8Download = null;
    }
